...
var app, echonest, express, http, implodeArtists, io, nest, playlistData, server, settings, spotify;
...
io.on('connection', function(socket) {
  ...
  socket.on('addTrack', function(data) {
    ...
    return playlistData.push(data);
  });
  socket.on('reqPlaylist', function(fn) {
    return fn(playlistData);
  });
  ...
  socket.on('upVote', function(index, amount) {
    socket.broadcast.emit('upVote', {
      index: index,
      amount: amount
    });
    return playlistData[index].up += amount;
  });
  socket.on('downVote', function(index, amount) {
    socket.broadcast.emit('downVote', {
      index: index,
      amount: amount
    });
    return playlistData[index].down += amount;
  });
